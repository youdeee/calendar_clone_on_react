...
const Calendar = () => {
  ...
  const weekDays = (startWeekDay) => {
    moment.locale('ja');
    return moment.weekdaysShort().concat(moment.weekdaysShort()).slice(startWeekDay, startWeekDay + numberOfweedDay);
  }
  ...
  const _calcStartAndEndDay = (thisMonth, startWeekDay) => {
    ...
    const startDay = startDayOfMonth.clone().startOf('week').add(startWeekDay, 'days')
    ...
    const endDay = endDayOfMonth.clone().endOf('week').add(startWeekDay, 'days').subtract(numberOfweedDay, 'days')
    ...
    <div className={classes.calendar}>
      <Box display="flex" className={classes.weekDay}>
        {weekDays(state.startWeekDay).map((weekDay) => (
          <Box flex="1 1 0%" textAlign="center" key={weekDay}>{weekDay}</Box>
        ))}
      </Box>
      <Box className={classes.days}>
        {dayRows.map((dayRow, i) => (
          <Box display="flex" style={{ height: `calc(100% / ${dayRows.length})` }} key={`dayRow-${i}`}>
            {dayRow.map((day) => {
